...
def tagare_weight(
    ...
    if centered_correlation:
        correlation_term = cross_correlation_weight(
            images, reference, std, eps
        ).abs_()
    else:
        correlation_term = cosine_sim

    image_norm_sq = torch.linalg.vector_norm(
        images, dim=image_dims
    ).square_()

    orth_norm_sq = image_norm_sq * (
        1.0 - cosine_sim.square()
    ).clamp_min_(0.0)
...
def tagare_weight_precomputed(
    ...
    denominator = (
        image_norms.clamp_min(eps)
        * reference_norm.clamp_min(eps)
    )
