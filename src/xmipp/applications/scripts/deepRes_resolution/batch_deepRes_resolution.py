...
def getBox(V, z, y, x):
    boxDim2 = boxDim // 2
    box = V[
        z - boxDim2:z + boxDim2 + 1, y - boxDim2:y + boxDim2 + 1, x - boxDim2:x + boxDim2 + 1]
...
def getDataIfFname(fnameOrNumpy):
    if isinstance(fnameOrNumpy, str):
        ...
        raise ValueError(
            "Error, input must be either file name or numpy array")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="determine the local resoluction")
    parser.add_argument("-dl", "--dl_model", help="input deep learning model",
                        required=True)
    ...
    parser.add_argument("-s", "--sampling", help="sampling rate",
                        required=True)
    parser.add_argument("-o", "--output", help="output resolution map",
                        required=True)
